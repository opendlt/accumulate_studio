# Building on Accumulate with the C# SDK

You are integrating the Accumulate blockchain using the C# SDK (`Acme.Net.Sdk`). Follow this guide.

## Golden path (use this, not the low-level API)
```
var client = Accumulate.Kermit();   // or Accumulate.Testnet()/Mainnet()/Devnet()
// 1. build the transaction body
// body = TxBody.<operation>(...)   // see Operations below
// 2. sign, submit, and wait for delivery
var r = await new SmartSigner(signer).SignSubmitAndWaitAsync(principal, body);
