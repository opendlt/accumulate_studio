...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let mut burncredits_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let burncredits_result = burncredits_signer.sign_submit_and_wait(
        "acc://e2e-1bf21fe3.acme/book/1",
        &TxBody::burn_credits(10),
        Some("Burn credits"),
        30,
    ).await;
    if burncredits_result.success {
        println!("BurnCredits SUCCESS - TxID: {:?}", burncredits_result.txid);
        if let Some(ref txid) = burncredits_result.txid {
            tx_ids.push(("BurnCredits", txid.clone()));
        }
    } else {
        println!("BurnCredits FAILED: {:?}", burncredits_result.error);
    }
