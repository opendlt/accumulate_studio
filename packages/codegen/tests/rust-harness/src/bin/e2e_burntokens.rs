...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-034c2026.acme".to_string();
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity(&createidentity_url, &format!("{}/book", createidentity_url), &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    let addcredits_2_result = addcredits_2_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::add_credits(&format!("{}/book/1", createidentity_url), "5000000", addcredits_2_price),
        Some("Add credits"),
        30,
    ).await;
    ...
    // =========================================================
    println!("Waiting for credits...");
    let waitforcredits_2_scope = format!("{}/book/1", createidentity_url).to_string();
    ...
    // =========================================================
    let createtoken_url = "acc://e2e-034c2026.acme/my-token".to_string();
    let mut createtoken_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let createtoken_result = createtoken_signer.sign_submit_and_wait(
        "acc://e2e-034c2026.acme",
        &TxBody::create_token(&createtoken_url, "TKN", 8, None),
        Some("Create token"),
        30,
    ).await;
    if createtoken_result.success {
        println!("CreateToken SUCCESS - TxID: {:?}", createtoken_result.txid);
        if let Some(ref txid) = createtoken_result.txid {
            tx_ids.push(("CreateToken", txid.clone()));
        }
    } else {
        println!("CreateToken FAILED: {:?}", createtoken_result.error);
    }

    // =========================================================
    // CreateTokenAccount (Action: CreateTokenAccount)
    // =========================================================
    let createtokenaccount_url = "acc://e2e-034c2026.acme/tokens".to_string();
    let mut createtokenaccount_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let createtokenaccount_result = createtokenaccount_signer.sign_submit_and_wait(
        "acc://e2e-034c2026.acme",
        &TxBody::create_token_account(&createtokenaccount_url, "acc://ACME"),
        Some("Create token account"),
        30,
    ).await;
    if createtokenaccount_result.success {
        println!("CreateTokenAccount SUCCESS - TxID: {:?}", createtokenaccount_result.txid);
        if let Some(ref txid) = createtokenaccount_result.txid {
            tx_ids.push(("CreateTokenAccount", txid.clone()));
        }
    } else {
        println!("CreateTokenAccount FAILED: {:?}", createtokenaccount_result.error);
    }

    // =========================================================
    // IssueTokens (Action: IssueTokens)
    // =========================================================
    let mut issuetokens_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let issuetokens_result = issuetokens_signer.sign_submit_and_wait(
        "acc://e2e-034c2026.acme/my-token",
        &TxBody::issue_tokens_single("acc://e2e-034c2026.acme/tokens", "1000000"),
        Some("Issue tokens"),
        30,
    ).await;
    if issuetokens_result.success {
        println!("IssueTokens SUCCESS - TxID: {:?}", issuetokens_result.txid);
        if let Some(ref txid) = issuetokens_result.txid {
            tx_ids.push(("IssueTokens", txid.clone()));
        }
    } else {
        println!("IssueTokens FAILED: {:?}", issuetokens_result.error);
    }

    // =========================================================
    // BurnTokens (Action: BurnTokens)
    // =========================================================
    let mut burntokens_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let burntokens_result = burntokens_signer.sign_submit_and_wait(
        "acc://e2e-034c2026.acme/tokens",
        &TxBody::burn_tokens("500"),
        Some("Burn tokens"),
        30,
    ).await;
    if burntokens_result.success {
        println!("BurnTokens SUCCESS - TxID: {:?}", burntokens_result.txid);
        if let Some(ref txid) = burntokens_result.txid {
            tx_ids.push(("BurnTokens", txid.clone()));
        }
    } else {
        println!("BurnTokens FAILED: {:?}", burntokens_result.error);
    }
