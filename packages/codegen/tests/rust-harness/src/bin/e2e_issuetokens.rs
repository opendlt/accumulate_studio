...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity("acc://e2e-1f9c2c24.acme", "acc://e2e-1f9c2c24.acme/book", &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    let createtoken_result = createtoken_signer.sign_submit_and_wait(
        "acc://e2e-1f9c2c24.acme",
        &TxBody::create_token("acc://e2e-1f9c2c24.acme/my-token", "TKN", 8, None),
        Some("Create token"),
        30,
    ).await;
    ...
    let createtokenaccount_result = createtokenaccount_signer.sign_submit_and_wait(
        "acc://e2e-1f9c2c24.acme",
        &TxBody::create_token_account("acc://e2e-1f9c2c24.acme/tokens", "acc://ACME"),
    ...
    let issuetokens_result = issuetokens_signer.sign_submit_and_wait(
        "acc://e2e-1f9c2c24.acme/my-token",
        &TxBody::issue_tokens_single("acc://e2e-1f9c2c24.acme/tokens", "1000000"),
        Some("Issue tokens"),
        30,
    ).await;
