...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-790f257b.acme".to_string();
    ...
    // =========================================================
    let mut updatekeypage_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let updatekeypage_result = updatekeypage_signer.sign_submit_and_wait(
        "acc://e2e-790f257b.acme/book/1",
        &TxBody::update_key_page(&json!([{"type":"add","key":"deadbeefdeadbeefdeadbeefdeadbeef"}])),
        Some("Update key page"),
        30,
    ).await;
    if updatekeypage_result.success {
        println!("UpdateKeyPage SUCCESS - TxID: {:?}", updatekeypage_result.txid);
        if let Some(ref txid) = updatekeypage_result.txid {
            tx_ids.push(("UpdateKeyPage", txid.clone()));
        }
    } else {
        println!("UpdateKeyPage FAILED: {:?}", updatekeypage_result.error);
    }
