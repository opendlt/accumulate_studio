...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-32b724ed.acme".to_string();
    ...
    // =========================================================
    let createdataaccount_url = "acc://e2e-32b724ed.acme/data".to_string();
    let mut createdataaccount_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let createdataaccount_result = createdataaccount_signer.sign_submit_and_wait(
        "acc://e2e-32b724ed.acme",
        &TxBody::create_data_account(&createdataaccount_url),
        Some("Create data account"),
        30,
    ).await;
    if createdataaccount_result.success {
        println!("CreateDataAccount SUCCESS - TxID: {:?}", createdataaccount_result.txid);
        if let Some(ref txid) = createdataaccount_result.txid {
            tx_ids.push(("CreateDataAccount", txid.clone()));
        }
    } else {
        println!("CreateDataAccount FAILED: {:?}", createdataaccount_result.error);
    }

    // =========================================================
    // WriteData (Action: WriteData)
    // =========================================================
    let writedata_entries: Vec<&str> = vec!["Hello World"];
    let mut writedata_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let writedata_result = writedata_signer.sign_submit_and_wait(
        "acc://e2e-32b724ed.acme/data",
        &TxBody::write_data_hex(&writedata_entries),
        Some("Write data"),
        30,
    ).await;
    if writedata_result.success {
        println!("WriteData SUCCESS - TxID: {:?}", writedata_result.txid);
        if let Some(ref txid) = writedata_result.txid {
            tx_ids.push(("WriteData", txid.clone()));
        }
    } else {
        println!("WriteData FAILED: {:?}", writedata_result.error);
    }
