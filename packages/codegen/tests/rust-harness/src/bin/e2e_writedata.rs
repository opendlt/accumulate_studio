...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity("acc://e2e-7484ecc4.acme", "acc://e2e-7484ecc4.acme/book", &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    // =========================================================
    let mut createdataaccount_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let createdataaccount_result = createdataaccount_signer.sign_submit_and_wait(
        "acc://e2e-7484ecc4.acme",
        &TxBody::create_data_account("acc://e2e-7484ecc4.acme/data"),
    ...
    // =========================================================
    let writedata_entries: Vec<&str> = vec![
        "Hello World",
    ];
    let mut writedata_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let writedata_result = writedata_signer.sign_submit_and_wait(
        "acc://e2e-7484ecc4.acme/data",
        &TxBody::write_data(&writedata_entries),
        Some("Write data"),
        30,
    ).await;
