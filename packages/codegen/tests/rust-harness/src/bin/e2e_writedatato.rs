...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let writedatato_entries: Vec<&str> = vec!["Hello World"];
    let mut writedatato_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let writedatato_result = writedatato_signer.sign_submit_and_wait(
        "acc://e2e-8fa5b79a.acme/data",
        &TxBody::write_data_to_hex("acc://e2e-8fa5b79a.acme/data", &writedatato_entries),
        Some("Write data to"),
        30,
    ).await;
    if writedatato_result.success {
        println!("WriteDataTo SUCCESS - TxID: {:?}", writedatato_result.txid);
        if let Some(ref txid) = writedatato_result.txid {
            tx_ids.push(("WriteDataTo", txid.clone()));
        }
    } else {
        println!("WriteDataTo FAILED: {:?}", writedatato_result.error);
    }
