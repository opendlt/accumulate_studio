...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-1859e8d5.acme".to_string();
    ...
    // =========================================================
    let createkeybook_url = "acc://e2e-1859e8d5.acme/book2".to_string();
    let mut createkeybook_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let createkeybook_result = createkeybook_signer.sign_submit_and_wait(
        "acc://e2e-1859e8d5.acme",
        &TxBody::create_key_book(&createkeybook_url, &generatekeys_pub_hash),
        Some("Create key book"),
        30,
    ).await;
    if createkeybook_result.success {
        println!("CreateKeyBook SUCCESS - TxID: {:?}", createkeybook_result.txid);
        if let Some(ref txid) = createkeybook_result.txid {
            tx_ids.push(("CreateKeyBook", txid.clone()));
        }
    } else {
        println!("CreateKeyBook FAILED: {:?}", createkeybook_result.error);
    }
