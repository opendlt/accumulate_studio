...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity("acc://e2e-221eb692.acme", "acc://e2e-221eb692.acme/book", &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    // =========================================================
    let mut createkeybook_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let createkeybook_result = createkeybook_signer.sign_submit_and_wait(
        "acc://e2e-221eb692.acme",
        &TxBody::create_key_book("acc://e2e-221eb692.acme/book2", "generatekeys_pub_hash"),
