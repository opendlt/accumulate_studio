...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-ddb106f8.acme".to_string();
    ...
    // =========================================================
    let mut updatekey_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let updatekey_result = updatekey_signer.sign_submit_and_wait(
        "acc://e2e-ddb106f8.acme/book/1",
        &TxBody::update_key("cafebabecafebabecafebabecafebabe"),
        Some("Update key"),
        30,
    ).await;
    if updatekey_result.success {
        println!("UpdateKey SUCCESS - TxID: {:?}", updatekey_result.txid);
        if let Some(ref txid) = updatekey_result.txid {
            tx_ids.push(("UpdateKey", txid.clone()));
        }
    } else {
        println!("UpdateKey FAILED: {:?}", updatekey_result.error);
    }
