...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity("acc://e2e-0f535b0f.acme", "acc://e2e-0f535b0f.acme/book", &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    let createtoken_result = createtoken_signer.sign_submit_and_wait(
        "acc://e2e-0f535b0f.acme",
        &TxBody::create_token("acc://e2e-0f535b0f.acme/my-token", "TKN", 8, None),
        Some("Create token"),
        30,
    ).await;
