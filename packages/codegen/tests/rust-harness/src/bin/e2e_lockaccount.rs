...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-7e7aed80.acme".to_string();
    ...
    // =========================================================
    // LockAccount (Action: LockAccount)
    // =========================================================
    let mut lockaccount_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let lockaccount_result = lockaccount_signer.sign_submit_and_wait(
        "acc://e2e-7e7aed80.acme",
        &TxBody::lock_account(1),
        Some("Lock account"),
        30,
    ).await;
    if lockaccount_result.success {
        println!("LockAccount SUCCESS - TxID: {:?}", lockaccount_result.txid);
        if let Some(ref txid) = lockaccount_result.txid {
            tx_ids.push(("LockAccount", txid.clone()));
        }
    } else {
        println!("LockAccount FAILED: {:?}", lockaccount_result.error);
    }
