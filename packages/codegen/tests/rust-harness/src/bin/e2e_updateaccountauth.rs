...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    //
    // UpdateAccountAuth E2E
    // E2E test for UpdateAccountAuth
    // Generated by Accumulate Studio
    //
    ...
    // =========================================================
    let createidentity_url = "acc://e2e-7b40e043.acme".to_string();
    ...
    // =========================================================
    let createtokenaccount_url = "acc://e2e-7b40e043.acme/tokens".to_string();
    ...
    // =========================================================
    // UpdateAccountAuth (Action: UpdateAccountAuth)
    // =========================================================
    let mut updateaccountauth_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &format!("{}/book/1", createidentity_url));
    let updateaccountauth_result = updateaccountauth_signer.sign_submit_and_wait(
        "acc://e2e-7b40e043.acme",
        &TxBody::update_account_auth(&json!([{"type":"enable","authority":"acc://e2e-7b40e043.acme/book"}])),
        Some("Update account auth"),
        30,
    ).await;
    if updateaccountauth_result.success {
        println!("UpdateAccountAuth SUCCESS - TxID: {:?}", updateaccountauth_result.txid);
        if let Some(ref txid) = updateaccountauth_result.txid {
            tx_ids.push(("UpdateAccountAuth", txid.clone()));
        }
    } else {
        println!("UpdateAccountAuth FAILED: {:?}", updateaccountauth_result.error);
    }
