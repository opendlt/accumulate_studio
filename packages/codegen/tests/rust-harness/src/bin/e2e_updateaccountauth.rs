...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let createidentity_result = createidentity_signer.sign_submit_and_wait(
        &generatekeys_lta,
        &TxBody::create_identity("acc://e2e-45105a05.acme", "acc://e2e-45105a05.acme/book", &generatekeys_pub_hash),
        Some("Create ADI"),
        30,
    ).await;
    ...
    let createtokenaccount_result = createtokenaccount_signer.sign_submit_and_wait(
        "acc://e2e-45105a05.acme",
        &TxBody::create_token_account("acc://e2e-45105a05.acme/tokens", "acc://ACME"),
    ...
    // =========================================================
    let mut updateaccountauth_signer = SmartSigner::new(&client, generatekeys_signer.clone(), &generatekeys_lid);
    let updateaccountauth_result = updateaccountauth_signer.sign_submit_and_wait(
        "acc://e2e-45105a05.acme",
        &TxBody::update_account_auth(&json!([{"type":"enable","authority":"acc://e2e-45105a05.acme/book"}])),
