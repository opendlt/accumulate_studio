...
async function main() {
    ...
    try {
        ...
        // =========================================================
        const createidentityUrl = "acc://e2e-eb972692.acme";
        ...
        // =========================================================
        const transfercreditsSigner = new SmartSigner(client, generatekeysKp.toKey(), `${createidentityUrl}/book/1`);
        const transfercreditsResult = await transfercreditsSigner.signSubmitAndWait(
            "acc://e2e-eb972692.acme/book/1",
            TxBody.transferCredits(
                "acc://e2e-eb972692.acme/book/1",
                100,
            ),
        );
        if (transfercreditsResult.success) {
            console.log(`TransferCredits SUCCESS - TxID: ${transfercreditsResult.txid}`);
            if (transfercreditsResult.txid) txIds.push(["TransferCredits", transfercreditsResult.txid]);
        } else {
            console.log(`TransferCredits FAILED: ${transfercreditsResult.error}`);
        }
