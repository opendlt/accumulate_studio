    ...
    try {
        ...
        const createidentityResult = await createidentitySigner.signSubmitAndWait(
            ...
            TxBody.createIdentity(
                "acc://e2e-b1d6d359.acme",
                "acc://e2e-b1d6d359.acme/book",
                generatekeysPubHash,
            ),
        );
        ...
        // =========================================================
        const updatekeypageSigner = new SmartSigner(client, generatekeysKp.toKey(), generatekeysLid);
        const updatekeypageResult = await updatekeypageSigner.signSubmitAndWait(
            "acc://e2e-b1d6d359.acme/book/1",
            TxBody.updateKeyPage([{"type":"add","key":"deadbeefdeadbeefdeadbeefdeadbeef"}]),
        );
