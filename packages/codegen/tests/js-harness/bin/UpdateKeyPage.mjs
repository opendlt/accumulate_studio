...
async function main() {
    ...
    try {
        ...
        // =========================================================
        const createidentityUrl = "acc://e2e-8c8ce4d9.acme";
        ...
        // =========================================================
        const updatekeypageSigner = new SmartSigner(client, generatekeysKp.toKey(), `${createidentityUrl}/book/1`);
        const updatekeypageResult = await updatekeypageSigner.signSubmitAndWait(
            "acc://e2e-8c8ce4d9.acme/book/1",
            TxBody.updateKeyPageAddKey(""),
        );
        if (updatekeypageResult.success) {
            console.log(`UpdateKeyPage SUCCESS - TxID: ${updatekeypageResult.txid}`);
            if (updatekeypageResult.txid) txIds.push(["UpdateKeyPage", updatekeypageResult.txid]);
        } else {
            console.log(`UpdateKeyPage FAILED: ${updatekeypageResult.error}`);
        }
