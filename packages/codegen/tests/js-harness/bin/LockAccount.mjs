...
async function main() {
    ...
    try {
        ...
        // =========================================================
        const createidentityUrl = "acc://e2e-330697c4.acme";
        ...
        // =========================================================
        const lockaccountSigner = new SmartSigner(client, generatekeysKp.toKey(), `${createidentityUrl}/book/1`);
        const lockaccountResult = await lockaccountSigner.signSubmitAndWait(
            "acc://e2e-330697c4.acme",
            TxBody.lockAccount(1),
        );
        if (lockaccountResult.success) {
            console.log(`LockAccount SUCCESS - TxID: ${lockaccountResult.txid}`);
            if (lockaccountResult.txid) txIds.push(["LockAccount", lockaccountResult.txid]);
        } else {
            console.log(`LockAccount FAILED: ${lockaccountResult.error}`);
        }
